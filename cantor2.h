...
Cantor* propagate(Cantor* head)
{
	...
	if (temp != NULL) {
		Cantor* newNode
			= new Cantor;
		double diff
			= (((temp->end) - (temp->start)) / cantor_divide_param );
...
std::vector<double> buildCantorSetOnCircle(int A, int B, int L, double div_par)
{
	cantor_divide_param = div_par;
