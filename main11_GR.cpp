...
int main(int argc, char *argv[])
{
	...
	for (const auto & entry : fs::directory_iterator(path))
	{
		...
		vector<vector<pair<bool,double>>> connectivity_matrix(max_num_wezla+1);

		for(int i=0; i<connectivity_matrix.size(); i++)
			connectivity_matrix.at(i) = vector<pair<bool,double>>(max_num_wezla+1, make_pair(false, 0.));
	...
	cout << endl << "sredni p_s dla N_component =" << N_component << " przy tych alfa i gamma (nie wiem jakich konkretnie) jest " << sredni_s_r << endl;
	/**/
	...
	string filename_grsummary = "grsummary_N_comp=" + to_string_with_precision(N_component) + "_gamma=unknown" + "_alfa=unknown" + ".txt";
