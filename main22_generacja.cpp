...
// main22 dodaje jako argumenty nazwe folderu docelowego i wskazanie, co zapisywac: fn - full network czy lc - largest component
...
int main(int argc, char *argv[])
{
	...
	string where_to_save = string("data_topologie_4");	
	if(argc == 8)
	...
	for(int iteracje=0; iteracje<max_iteracji; iteracje++)
	{
		...
		*/
		...
		else if( what_to_save == "lc" )
		
			for(int i=0; i<N; i++)
			{
				for(int j=i+1; j<N; j++)
				{
					//cout << (what_to_save == "fn") << endl;
