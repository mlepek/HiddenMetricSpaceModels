...
// main24_GR zamieniam 3. argument na pelna nazwe katalogu
...
int main(int argc, char *argv[])
{
	...
	if(argc != 1 and argc !=2 and argc!=3 and argc!=4)
	{
		...
		cerr << "1. nazwa folderu, 2. liczba powtorzen GR dla każdej instancji sieci, 3. wielkosc sieci N " << endl; 
		return 1;
	}



	string nazwa_folderu = string("data_topologie_1");		
	if(argc >= 2)
		nazwa_folderu = string(argv[1]);
	...
	std::string path = "./" + nazwa_folderu; // uwaga, zmieniajac te sciezce pamietaj zmienic "extract erase", linie 373+
	...
	*/
	...
	for (const auto & entry : fs::directory_iterator(path))
	{
		...
		//cout << extract << endl;
		//extract.erase(0, 27);
		string to_find = "network_";
		size_t found = extract.find(to_find);
		extract = extract.substr(found+8);
		//cout << extract << endl;
		to_find = "_";
		found = extract.find(to_find);
