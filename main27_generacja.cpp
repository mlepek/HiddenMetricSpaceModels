...
// main27 w tym main, w przypadku zapisu najwiekszego komponentu, renumeruje wezly w taki sposob, zeby nie mialy przerw w numeracji - potrzebne do testu prawidlowego dzialania wczytywania w networkit - jednak okazalo sie ze dlugo trwa i jest niekonieczne 
...
int main(int argc, char *argv[])
{
	...
	for(int iteracje=0; iteracje<max_iteracji; iteracje++)
	{
		...
		*/
		...
		cout << "zapisuje liste polaczen" << endl;
		
		string filename_topology = where_to_save + "/network_" + to_string(iteracje) + "_topology.edgelist";
		...
		*/
		...
		// sposob specjalnie dla listy polaczen
		...
		// renumeracja wezlow tak, by wezly w najwiekszych komponencie mialy numery od 0 do N_lc bez przerw - potrzebne do wczytywania w networkit
		
		//zbudujmmy "slownik" stara nazwa : nowa nazwa
		
		// argumenty wektora 0 do (N-1) sa to stare nazwy, w komorkach wektora beda nowe nazwy
		// w przypadku gdy dany wezel nie wystepuje w najwk. klastrze, otrzyma nowa nazwe "-1" 
		
		cout << "tworze slowniki nazw wezlow" << endl;
		
		vector<int> nowe_nazwy(N);
		int licznik_wezlow_lc = 0;
		
		for(int node_i=0; node_i<N; node_i++)
		{
			// jesli nalezy do najw. komponentu
			if( membership_of_monomers.at(node_i) == etykieta_najwiekszego_klastra )
			{
				nowe_nazwy.at(node_i) = licznik_wezlow_lc;
				licznik_wezlow_lc++;
			}
			else
				nowe_nazwy.at(node_i) = -1;
		}
		
		int N_lc = licznik_wezlow_lc;
		
		
		// tworze jeszcze slownik: nowa nazwa - stara nazwa
		
		vector<int> stare_nazwy(0);
		
		for(int i=0; i<N; i++)
			if( nowe_nazwy.at(i) >= 0 )
				stare_nazwy.push_back(i);
				
		cout << "zgodnosc rozmiarow slownikow: " << (bool)(N_lc == stare_nazwy.size()) << endl;
		cout << "rozmiar stare_nazwy: " << stare_nazwy.size() << endl;
		cout << "rozmiar nowe_nazwy: " << nowe_nazwy.size() << endl;
		cout << "N_lc: " << N_lc << endl;
		
		/*
		for(auto a:stare_nazwy)
			cout << a << "  ";
		cout << endl << endl;
		
		for(auto a:nowe_nazwy)
			cout << a << "  ";
		cout << endl << endl;
		*/
		...
		// teraz w wektorze nowe_nazwy mamy nowe nazwy wezlow z najwiekszego komponentu
		
		// przepiszmy cala liste polaczen na nowe nazwy
		
		// zakladam nowa liste polaczen
		
		cout << "renumeruje wezly w najw. komponencie" << endl;
		
		list<vector<unsigned>> connectivity_list_lc_renum( N_lc, vector<unsigned>(0) );
		
		it_i = connectivity_list_lc_renum.begin();
		
		for(int i=0; i<N_lc; i++, it_i++)
		{
			if ( i%10 == 0 ) cout << " " << double(i)/N_lc*100 << " % " << endl;
			
			
			list<vector<unsigned>>::iterator it_j = connectivity_list_lc_renum.begin();
	
			advance(it_j, i);
			
			
			//for(int j=i+1; j<nodes_.size(); j++) // we treat any pair only once and do not treat i=j
			for(int j=i; j<N_lc; j++, it_j++) 
			{
				// nie zakladamy polaczen do samego siebie
				if(i==j) continue;
				
				// jesli wezly nowe, ktore rozpatrujemy, byly polaczone, to musimy je polaczyc takze w nowej renumerowanej liscie polaczen
				
				//cout << "przed wzieciem starych nazw " << endl;
				
				//cout << "i:" << i << " , j:" << j << " " << endl;
				
				int node_i_stara_nazwa = stare_nazwy.at(i);
				int node_j_stara_nazwa = stare_nazwy.at(j);
				
				//cout << "po wzieciu starych nazw " << endl;
				
				// sprawdzam czy istnialo polaczenie pomiedzy wezlami ze starymi nazwami
				// jesli tak to dodaje polaczenie w nowej liscie polaczen
				if( check_connection( connectivity_list, node_i_stara_nazwa, node_j_stara_nazwa ) )
				{
					// dodajmy w wektorze wezla i sasiada j
					(*it_i).push_back(j);
		...
		// teraz w connectivity_list_lc_renum mam liste polaczen najwiekszego klastra renumerowana
		
		// zapiszmy ja 
		
		cout << "zapisuje liste polaczen najw. klastra renumerowana" << endl;
		
		string filename_topology_lc_renum = where_to_save + "/network_" + to_string(iteracje) + "_topology_lc_renum.edgelist";
		ofstream output_topology_lc_renum(filename_topology_lc_renum.c_str());
		
		output_topology_lc_renum << setprecision(8);
		
		node_i = 0;
		
		for(list<vector<unsigned>>::const_iterator it = connectivity_list_lc_renum.begin(); it != connectivity_list_lc_renum.end(); ++it)
		{
			for(int j=0; j<(*it).size(); j++)
			{
				int node_j = (*it).at(j);
				output_topology_lc_renum << node_i << " " << node_j << " " << d_theta_theta(nodes_.at(stare_nazwy.at(node_i)).theta, nodes_.at(stare_nazwy.at(node_j)).theta, R) << endl;
				//output_topology_lc_renum << node_i << " " << node_j << endl;
			}		
			node_i++;
		}
		
		
		output_topology_lc_renum.close();
		/**/
