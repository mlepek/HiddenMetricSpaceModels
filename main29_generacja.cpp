...
// main29 wersja specjalnie do generacji punktow na kole z jednakowym stopniem 1 i z rownomiernym rozlozeniem na kole - do sprawdzenia, czy wymiar fraktalny wyjdzie 1 - bedziemy badac tylko siec "alldists"
	...
	for(int iteracje=0; iteracje<max_iteracji; iteracje++)
	{
		...
		if( dzielnik_Cantora <= 2. )
		{
			...
			{				
				...
				n.theta = current_position;
				current_position += krok;
			}
		}
		else
		{
		...
		for(auto& n : nodes_)
		{
			...
			//specjalnie dla tej proby
			n.kappa = 1.;
			// jak to sie w rzeczywistosci polaczy, nie ma za bardzo znaczenia, bo dla uzyskania koordynatow hiperbolicznyhc i tak uzyjemy tylko kapp 
		}
