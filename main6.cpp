...
// w tym mainie, main6,  zmieniam losowanie wezlow do GR tak, zeby zaden z wysolosowanych wezlow nie mial stopnia zero
...
int main(int argc, char *argv[])
{
	...
	for(int iteracje=0; iteracje<max_iteracji; iteracje++)
	{
		...
		string filename = "network_" + to_string(iteracje) + "_nodes.txt";
		...
		*/
		...
		string filename_topology = "network_" + to_string(iteracje) + "_topology.txt";
		...
		*/
		...
		for(int numer_gr = 0; numer_gr < max_gr; numer_gr++)
		{
			...
			while(wezel_b == wezel_a or nodes_.at(wezel_a).k == 0 or nodes_.at(wezel_b).k == 0);
	...
	// czasami faktyczne stopnie wezlow wychodza zero - czy tak ma byc? sprawdzic. moze z tego wynika pomniejszony success ratio greedy routing
